...
\geometry{a4paper,left=20mm,right=20mm, top=25mm, bottom=28mm} 
\usepackage{graphicx}
\usepackage{amssymb}
\usepackage{epstopdf}
\usepackage{natbib}
...
\\
     \large{Thomas Hahn}  & \large{Severin Jäger} & \large{Yannick Schubert}
\end{tabular}
\end{table}

\end{comment}

\includepdf[pages={1}]{figures/scan0018.pdf}
...
\abstract
This paper discusses a very powerful model that classifies the spread of infectious diseases. It provides the tools with which we can predict whether or not a disease will turn into an epidemic. In our work, we are able to show the underlying concept, as well as validate the model experimentally for practically relevant cases. Additionally, we found circumstances, under which the model reaches its limits.
\bigskip
...
Thomas Hahn mainly focuses on the theory behind the model. This includes understanding when the model can potentially fail and why it works otherwise.  Yannick Schubert and Severin Jäger implement the theory and verify results by simulating the time evolution of the SIR model and comparing outcomes.  In addition, they analyze what happens when certain important nodes are infected.

\section{Introduction and Motivations}
The social sciences have long tried to predict the time evolution propagation of viruses, ideas, etc.  Obviously, a deep understanding of how and why diseases spread would allow us to better come up with prevention measures.  Their first attempt was to parametrize the disease and analyze the spreading during short time-steps.  The topology of the system, i.e. the structure that describes the connection between all players (or lack thereof), was completely ignored.  It was just assumed that everyone knew everyone.  More recently, experts in this field have managed to not only create a theory that includes both aspects, but also achieves a decoupling.  This, of course, is by no means trivial.

We will base our work on the paper 'Threshold Conditions for Arbitrary Cascade Models on Arbitrary Networks" from 2012 \cite{original_paper}. They claim to be the first ones to decouple the virus propagation model from the topology of the network. There, they define a virus strength as the product of the largest eigenvalue of the connectivity matrix and a constant, which only depends on the virus propagation model.

The G2-Threshold Theorem, which is formulated and proven in the paper, states that there exists a threshold for the effective strength below which a virus dies out quickly. For many “what-if”-scenarios this is already the necessary information. Since there exist very fast algorithms for computing the largest eigenvalue of a matrix, this information can be obtained quickly for an arbitrary virus propagation model and an arbitrary topology.

In this paper, we will investigate whether the G2-Theorem still applies for different initial conditions. Thereto, we will vary the number of initially infected nodes in a network and infect its most important nodes. Additionally, we will figure out, whether virus propagation in very small networks can still be adequately analyzed with this theorem.

\newpage

\section{Description and Proof of the G2-Threshold Theorem} \label{theorem}
For any given epidemiological model, there are two key components that will describe our system; namely the virus propagation model and the topological constraints of the system.  It will be shown that these two components can in fact be decoupled and described by two constants.  To fully describe arbitrary systems, some definitions must be introduced. \\
...
Throughout this paper, the connectivity matrix will fully describe the interaction between two different people.   \\
...
For any given VPM that can be thought of as a $S^* I^2 V^*$ model (there are two infectious states, but no constraints on the number of susceptible and vigilant states), which operates on an arbitrary topology, the virus will be eradicated quickly if 
 \begin{eqnarray}
 s < 1
 \end{eqnarray}
...
We start by defining a vector $\textbf{P}_t$ that will describe the system at time $t$.  In a model where $m$ is the total number of states and $n$ is the total number of nodes, this vector will have $m \cdot n$ entries, i.e. $\bold{P}_t \in \mathbb{R}^{m \cdot n} $. More specifically, we can write $\bold{P}_t$ in the following way: \\
\begin{eqnarray}
...
\begin{eqnarray}
P_{U,i,t+1} = \sum_{K} \alpha_{KU}  P_{K,i,t}. \label{eq: 11}
\end{eqnarray}
Equations (\ref{eq: 9}) to (\ref{eq: 11}) now define $g$, which is what we wanted.\\
...
This can be interpreted as the probability of any given node $i$ being in the Susceptible class.
 ...
 Here, we rely on equations (\ref{eq: 9}) to (\ref{eq: 11}) to express $g$ and then take all of the partial derivatives.  As all of our $n$ nodes can be elements of $m$ possible states, we expect our Jacobian matrix to be of the form $(m \cdot n) \times (m \cdot n)$.  By plugging in the values, we get the following results:\\
 \begin{table}[H]
     \centering
...
\captionsetup{width = 0.8\textwidth}
     \caption{Denotes the Components of The Jacobian. The first row and column represent the states}
     \label{tab: Jacobian 1}
 \end{table}
 ...
 \begin{eqnarray}
 \forall K \neq E,I : \alpha_{KE} = \alpha_{KI} = 0 \label{eq: 17}
 \end{eqnarray}
...
\end{tabular}
    \label{tab: Jacobian 2}
\captionsetup{width = 0.8\textwidth}
    \caption{Here we set the constants of equation (\ref{eq: 17}) to $0$}
\end{table}
...
As we can see in Table $2$, the Jacobian is of the form
 \begin{eqnarray}
  \mathcal{J}=
  \left( {\begin{array}{cc}
  \bold{B}_1 & \bold{B}_2 \\
...
Since   $\bold{v}$ is an eigenvector, we know that
 ...
 This, in turn, can be written as 
   \begin{eqnarray}
   \bold{B}_1   \bold{v}_1 +    \bold{B}_2   \bold{v}_2 &=& \lambda_ \mathcal{J}  \bold{v}_1\label{eq: 21} \\
...
\subsubsection{Case (i)}
  Scenario $(i)$ simplifies equation (\ref{eq: 21}) considerably, as the second term is dropped and we are left with 
\begin{eqnarray}
...
which then again again implies one of the following: \\
  ...
  Since $\bold{v}_2=0$ by assumption, $ \bold{v}_1=0$ would yield a zero vector as the solution to equation (\ref{eq: 20}), which is not a very interesting outcome (as $ \bold{v}=0 $ would not be considered an eigenvector).  We can therefore consider  $ \bold{v}_1$ to be an eigenvector of $\bold{B}_1$.  \\
  \underline{Definition:}
 ...
 \left( 1 - \sum_{K \neq S_y,E} \right)  & \alpha_{KS_y} &  \cdots\\
  ...
  Here, we know that $\bold{v}_2$ is an eigenvector of $\bold{B}_3$.  From Table $2$ it follows that 
  \begin{eqnarray}
\bold{B}_3 =   \left( {\begin{array}{cc}
  ...
  \end{array} } \right) .
  \end{eqnarray}
  For pragmatic reasons, we shall split $\bold{v}_2$ into two components, $\bold{u}_1$ and $\bold{u}_2$, such that both are vectors of length $n$.  Since $\bold{v}_2$ is an eigenvector with eigenvalue $\lambda_ \mathcal{J}$, we know that the two components satisfy:
  \begin{eqnarray}
  \left(  \alpha_{EE} \mathbbm{1} + p_S^{\star} \beta_1 \bold{A} \right) \bold{u}_1 + \left( \alpha_{IE} \mathbbm{1} + p_S^{\star} \beta_2 \bold{A} \right)  \bold{u}_2 &=& \lambda_ \mathcal{J} \bold{u}_1 \label{eq: 28}\\ 
   \alpha_{E1} \bold{u}_1 +    \alpha_{II} \bold{u}_2 &=& \lambda_ \mathcal{J} \bold{u}_2 \label{eq: 29}
  \end{eqnarray}
  Equation (\ref{eq: 29}) tells us that 
  \begin{equation}
  \bold{u}_1 = \left( \frac{\lambda_ \mathcal{J} -\alpha_{II}}{\alpha_{EI}} \right)  \bold{u}_2 ,
  \end{equation}
  which can be plugged into (\ref{eq: 28}) to get:
  \begin{eqnarray}
  \left( \left( \alpha_{EE} \mathbbm{1} + p_S^{\star} \beta_1 \bold{A} \right) \left( \frac{\lambda_ \mathcal{J} -\alpha_{II}}{\alpha_{EI}} \right)  + \left( \alpha_{IE} \mathbbm{1} + p_S^{\star} \beta_2 \bold{A} \right) \right)  \bold{u}_2 = \lambda_ \mathcal{J} \left( \frac{\lambda_ \mathcal{J} -\alpha_{II}}{\alpha_{EI}} \right)  \bold{u}_2 \label{eq: 31}
  \end{eqnarray} 
  We can now reorder the equation such that we end up with:
  \begin{eqnarray}
  \bold{A} \bold{u}_2 = \left( \frac{\lambda_ \mathcal{J}^2 - \left(\alpha_{II} + \alpha_{EE} \right)  \lambda_ \mathcal{J} + \alpha_{II}  \alpha_{EE} - \alpha_{IE}  \alpha_{E1}}{p_S^{\star} \beta_1 \left(\lambda_ \mathcal{J} -\alpha_{II} \right) + p_S^{\star} \beta_2 \alpha_{EI}  } \right)  \bold{u}_2 \label{eq: 32}
  \end{eqnarray}
  Since we know that $ \bold{u}_2  \neq 0$ (otherwise $ \bold{u}_1 =0$ and $ \bold{v} = 0$),  $ \bold{u}_2 $ is an eigenvector of the connectivity matrix. As we can see in (\ref{eq: 32}), the eigenvalue is dependent on many parameters.  To simplify matters, we shall call the eigenvalue $\lambda_{\bold{A}}$.  We can now replace $\bold{A}$ with its eigenvalue in equation (\ref{eq: 31}), so that it can be rewritten as:
  \begin{eqnarray}
...
+  p_S^{\star} \lambda_{\bold{A}} \left( \beta_1 \alpha_{II} - \beta_2 \alpha_{EI} \right) \right) =0 \label{eq: 33}
\end{eqnarray}
...
\underline{Lemma:} \\
The eigenvalues of $\mathcal{J}$ are either acquired via the matrix $\bold{T}$ or by solving the quadratic equation (\ref{eq: 33}), whose roots are dependent in the eigenvalues of $\bold{A}$.
...
$(i)$ The eigenvalues of $\bold{T}$ must all have an absolute value less than $1$. \\
$(ii)$ The solutions of the quadratic equation given by (\ref{eq: 33}) must all have an absolute value less than $1$ (for all eigenvalues of $\bold{A}$). \\
Let's first analyze scenario $(i)$.
\subsubsection{Case $(i)$}
...
In this case, we must find the conditions for which our eigenvalues have an absolute value less than $1$.  To do this, we will prove the following lemma: \\
 \underline{Lemma:}  \\
 The solutions of the quadratic equations, which are given by (\ref{eq: 33}) for all eigenvalues of $\bold{A}$, have an absolute value less than $1$ if the following inequality holds:
 \begin{eqnarray}
 \lambda_1 p_S^{\star} \left( \frac{\beta_1 \left( 1- \alpha_{II}\right) + \beta_2 \alpha_{EI}}{\left( 1- \alpha_{II}\right) \left( 1- \alpha_{EE}\right) - \alpha_{IE}  \alpha_{E1}} \right) < 1,
 \end{eqnarray}
 
where $\lambda_1$ is the largest eigenvalue of $\bold{A}$. \\
 \underline{Proof:}  \\
 We define $r_1$ and $r_2$ to be the solutions of equation (\ref{eq: 33}) for a given $\lambda_{\bold{A}}$.  We obviously want that
 \begin{eqnarray}
 |r_1| < 1 \  \& \ |r_2| < 1 .
 \end{eqnarray}
...
\end{eqnarray}
These follow from the fact that all the parameters must be positive and less than or equal to $1$.  With these two inequalities, it should be clear that if (\ref{eq: 43}) holds, (\ref{eq: 42}) must hold as well.  We can thus conclude, that if the roots of the quadratic equation are real
\begin{eqnarray}
...
\end{eqnarray}
By rearranging the equation though, it turns out that this gives us equation (\ref{eq: 42}).  We already know though, that (\ref{eq: 42}) is implied by (\ref{eq: 43}).  Thus, if the condition given by (\ref{eq: 43}) is fulfilled, the roots must have an absolute value of less than $1$.
...
\begin{enumerate}
\item[$(i)$] An infected node infects a susceptible neighbour 
\item[$(ii)$] An infected node gets recovered
\end{enumerate}
Hereinafter we will call the set of susceptible nodes that have at least one infected neighbor "at\_risk\_nodes". 
The model is entirely described by the transmission probability over a contact-link $\beta$ and the healing probability once infected $\delta$. The parameter $\beta$ is defined such that the probability that $(i)$ happens in the infinitesimal time interval $[t, t+dt)$ is given by $\beta dt$. Analogously the probability that an infected node at time $t$ gets healed (event $(ii)$) within the time interval $[t,t+dt)$ is $\delta dt$. Therefore the probability that exactly one event happens in the time interval $[t,t+dt)$ is given by 
\begin{align}
...
\end{itemize}
\end{enumerate} 
Since $r_1$ is a random number uniformly distributed in $(0,1)$, the probability that $r_1<\text{total\_recovery\_rate}\times\Delta t$ is equal to $\text{total\_recovery\_rate}\times\Delta t$. So for infinitely small $\Delta t$, 2 (a) would exactly simulate the probability that an event will happen in the time step $t+\Delta t$. If an event happens in the considered time interval, the next  question is which event that is. The probability that a node recovers during this time-step is given by the probability that $\text{total\_recovery\_rate} <r_2$. In the remaining cases this event will be a transmission of the disease. The probability that a at\_risk\_node gets infected is simply proportional to its number of infected neighbors. \\
The problem with this algorithm is that by decreasing $\Delta t$ (and therefore increasing the accuracy), the computational intensity increases. During most of the time steps, the generated random number $r_1$ is created, even though no event took place.\\
The Gillespie follows a different approach: Instead of analyzing each time step, it calculates the time $t+\tau$ at which the next event occurs. It is a model that was developed in the middle of the $20^{th}$ century and was mainly used to efficiently simulate chemical or biochemical reactions \cite{wiki:Gillespie}. Instead of explaining the general algorithm, we want to focus on how it can be used to simulate the SIR model. Our paper will closely follow the work done in \cite{paper_gillespie}, where it is shown how the Gillespie algorithm can be used to describe chemical reactions.
\\
We shall define $f(\text{total\_rate}(t), s)ds$ as the probability that for a given $\text{total\_rate}$ at time $t$, the next event will happen during the time interval $[t+s,t+s+ds)$ and $g(\text{total\_rate}(t),s)$ as the probability, that no event happens in the time interval $[t,t+s)$. \\
The probability that for a given time $t$ the next event will happen during the time interval $[t+s,t+s+ds)$ can now be written as the product of the probabilities, that no event happens in the time interval $[t,t+s)$ and the probability that exactly one event happens in the time interval $[t+s,t+s+ds)$: $f(\text{total\_rate}(t), s)ds = g(\text{total\_rate}(t), s)\times\text{total\_rate}(t+s)ds$. Since no event happens in $[t,t+s)$ we know that $\text{total\_rate}(t+s)=\text{total\_rate}(t)$. Therefore: 
\begin{align}
...
\end{align}
With the initial condition that $g(\text{total\_rate}(t), 0)=1$, we obtain the unique solution $g(\text{total\_rate}(t), \sigma)=\exp[-\text{total\_rate}(t)\times\sigma]$. From this we know that \\
$f(\text{total\_rate}(t), s)ds\overset{(\ref{f})} = \exp[-\text{total\_rate}(t)s]\times\text{total\_rate}(t)ds$. Therefore the time $\tau$, such that the next event happens at the time $t+\tau$, is exponentially distributed according to the probability distribution $f(\lambda, x)=\lambda\exp[-\lambda x]$ where $\lambda = \text{total\_rate}(t)$. \\
A Pseudocode for the Gillespie algorithm, which can be used to simulate disease outbreaks with the SIR model, is shown in figure \ref{figure: Gillespie-pseudo}.\\

\begin{figure} [H]
\centering
\includegraphics[width=0.75\textwidth]{figures/pseudocode}
...
\item Compute the time $t+\tau$ at which the next event will happen. For this purpose the random number $\tau$ exponentially distributed according to $f(s) = \exp[-\text{total\_rate}(t)s]\times\text{total\_rate}(t)$ is generated
\item  Compute which event will happen
\begin{itemize}
\item[(i)] If $\text{total\_recovery\_rate} < r_2$: choose a random infected node $u$, set its state to "recovered" and reduce the number\_of\_infected\_neighbors[$\nu$] for every neighbor $\nu$ of $u$ by 1
\item[(ii)] Else: Choose $u$ from at\_risk\_nodes with probability \\
$\frac{\beta\times\text{number\_of\_infected\_neighbours}[u]}{\text{total\_infection\_rate}}$, remove it from the at\_risk\_nodes, set its state to infected and add its neighbors to at\_risk\_nodes
\end{itemize}
...
\bold{A}x=\lambda x
\end{equation}
where $\mathbf{A}$ denotes the networks’ connectivity matrix and $\lambda$ denotes the largest eigenvalue of $\mathbf{A}$, which is real and positive by the Perron-Frobenius theorem. Hence, $x$ is the eigenvector belonging to the spectral radius of $\mathbf{A}$. \cite{wiki:centrality}
\subsection{Implementation}
To simulate the infections, we modified an implementation of the Gillespie algorithm from \cite{Code-Gillespie}, as well as Python functions, which enabled us to plot the time behaviour of the infection. In addition we computed the final number of cured nodes (footprint) as a function of the effective strength. Our Code uses the libraries scipy, matplotlib, networkX, and EoN.
\subsubsection*{Data sets}
For the experiments described in the following section, we use the following data sets.
\begin{enumerate}
\item[$(i)$] AS-OREGON: Internet connection data from January 2, 2000 collected by University of Oregon Route Views Project \cite{as-oregon}. $6474$ nodes, $13895$ edges, spectral radius $46.9$.
\item[$(ii)$] FACEBOOK-EGO: Data from a survey on Facebook app usage \cite{facebook-ego}. $4039$ nodes, $88234$ edges, spectral radius $162.4$.
\item[$(iii)$] TERRORIST: Social associations between the terrorists involved in the 9/11 attacks \cite{terrorists}. $62$ nodes, $304$ edges, spectral radius $8.7$.
\end{enumerate}
\subsection{Simulation Results and Discussion}
For our experiments, we conducted computer simulations using Python 3.7. To create meaningful plots, we ran at least $200$ independent simulations per figure and plotted their arithmetic mean.

\begin{figure}[h]
\centering
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=\textwidth]{figures/fig1a_time_rnd.png}

\caption{}
\end{subfigure}
%
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=\textwidth]{figures/fig1b_time_crucial.png}
\caption{}
\end{subfigure}
\captionsetup{width = 0.75\textwidth}
\caption{Time behaviour of the SIR simulations for random \textbf{(a)} and critical \textbf{(b)} initial infection on the AS-OREGON network.}
\label{figure: fig1}
\end{figure}
\subsubsection*{Reproduction of previous work}
To reproduce the work in \cite{original_paper}, we used the AS-OREGON data set, which is similar to one of the data sets in their experiments. Initially we infected $10$ random nodes and investigated the time behaviour of the SIR simulation, which is shown in figure \ref{figure: fig1}a. The result matches the expectations, as s-Values greater $1$ lead to a significant increase of infected nodes, while under-critical infections ($s < 1$) die out quickly. This behavior is confirmed by the simulation results shown in figure \ref{figure: fig2}, which support the assumption that the tipping point is at $s=1$.

\begin{figure}[h!]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig2_tip_rnd.png}
\captionsetup{width = 0.75\textwidth}
\caption{Infection footprint for different SIR virus strengths on the AS-OREGON network.}
\label{figure: fig2}
\end{figure}

\subsubsection*{Initial infection size}
As we were able to reproduce the results from \cite{original_paper}, we investigated the influence of the size of the initially infected population. We therefore ran the same simulation from before, but randomly altered the initial number of infected nodes. The result is shown in figure \ref{figure: fig3}a, which suggests, that the model is still valid, even for notable sizes of initially infected nodes. However, the tipping point slightly moved to the left with increasing numbers of initial infections.

\begin{figure}[h]
\centering
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=\textwidth]{figures/fig3a_tip_rnd_init.png}

\caption{}
\end{subfigure}
%
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=\textwidth]{figures/fig3b_tip_crit_init.png}
\caption{}
\end{subfigure}
\captionsetup{width = 0.75\textwidth}
\caption{SIR infection footprint for different sizes of the random \textbf{(a)} and critical \textbf{(b)} initial infection on the AS-OREGON network.}
\label{figure: fig3}
\end{figure}

\subsubsection*{Infection of important nodes}
In the next step, we initially infected the nodes with the highest eigenvector centrality (see section \ref{centrality}). As expected, this is beneficial to the spreading of the virus. Figure \ref{figure: fig4} compares the footprint of random infection with critical node infection. We can clearly see that the initial infection of the nodes with the highest eigenvalue centrality moves the tipping point slightly to the left of $1$. Hence, infection of critical nodes brings the model already close to its limit. This fits the time behavior of the system, which is shown in figure \ref{figure: fig1}b, where the below-critical infections spread slightly before dying out. 

Additionally, we conducted experiments with larger numbers of critical initial infections. The results are shown in figure \ref{figure: fig3}b and again indicate that by initially infecting nodes with the highest centrality, we create a tipping point very close to $1$, particularly for larger numbers of initially infected nodes. However, the model can still be considered valid, as the footprint for effective strengths below $1$ remains very close to the initial infection. This meets, due to the generality of the proof (see section \ref{proof}), our expectations.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig4_rnd_vs_crit.png}
\captionsetup{width = 0.75\textwidth}
\caption{Infection footprint for different SIR virus strengths for random (blue) and critical (orange) initial infection on the AS-OREGON network.}
\label{figure: fig4}
\end{figure}


\subsubsection*{Verification with highly connected network}
To verify the results of the previous section, we ran the same simulation on a different network. We used the FACEBOOK-EGO network, which has a slightly smaller number of nodes, but nevertheless a larger spectral radius and therefore a higher density of connections. For this data set we were able to reproduce all the outcomes from the AS-OREGON network. As expected, the model is also valid for all our test cases on this network.
\subsubsection*{Behavior of small networks}
As the behavior for very large graphs was already investigated in \cite{original_paper}, we were interested if the model is also applicable for small networks. We therefore ran simulations on the TERRORIST network with $62$ nodes and a high density of connections. We again infected the nodes with the highest eigenvector centrality. Figure \ref{figure: fig5} shows the results for different sizes of the initial infection. Now the tipping point is, especially for larger numbers of initially infected nodes, below one. This is interesting, as for the larger networks even higher ratios of initially infected nodes led to no disease spreading. Hence, we draw the conclusion that the model is not generally valid for small networks.
\begin{figure}[h]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig5_terror.png}
\captionsetup{width = 0.75\textwidth}
\caption{Infection footprint for different numbers of initially infected nodes on the TERRORIST network.}
\label{figure: fig5}
\end{figure}

\section{Summary and Outlook}
In \autoref{theorem}, we saw that the G2 theorem is a powerful tool, as it creates a simple condition for analyzing VPMs on arbitrary networks and is almost always applicable (This is because the assumptions were reasonable and therefore do not really count as constraints). We clearly saw the vast scope of its utility in our experimental results. Only for small networks did we observed scenarios where the tipping point was off.

However, our experiments were limited to the SIR virus propagation model. It could very well be that more sophisticated VPMs may lead to different results, in which case the G2 theorem would cease to hold.

Another thing that has to be kept in mind is the fact the results are averages over many runs. We saw that particular simulations were strongly deviated from the mean. By taking the average over many peaks that do not align, the resulting curve will have a lower, wider peak which does not resemble a single realization \cite{paper_gillespie}.

Nevertheless, the G2-threshold theorem has shown to be very robust and reliable for the SIR model and there is no reason to suspect that this would be any different for other prevalent virus propagation models.
