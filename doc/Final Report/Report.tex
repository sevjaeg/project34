...
The social sciences have long tried to predict the time evolution propagation of viruses, ideas, etc.  Obviously, a deep understanding of how and why diseases spread would allow us to better come up with prevention measures.  Their first attempt was to parametrize the disease and analyze the spreading during short time-steps.  The topology of the system, i.e. a structure that describes the connection between all players (or lack thereof), was completely ignored.  It was just assumed that everyone knew everyone.  More recently, experts in this field have managed to not only create a theory that includes both aspects, but also achieve a decoupling.  This, of course, is by no means trivial.

We will base our work on the paper 'Threshold Conditions for Arbitrary Cascade Models on Arbitrary Networks" from 2012 \cite{original_paper}. They claim to be the first ones to de-couple the virus propagation model form the topology of the network. There, they define a virus strength as the product of the largest eigenvalue of the connectivity matrix and a constant, which only depends on the virus propagation model.
...
$(ii)$ $\gamma$ is the immunization loss probability after a node has recovered \\
...
\captionsetup{width = 0.8\textwidth}
     \caption{Denotes the Components of The Jacobian. The first row and columns represent the states}
...
\subsubsection{Case (i)}
  Scenario $(i)$ simplifies equation (\ref{eq: 21}) considerably, as the second term is dropped and we left with 
\begin{eqnarray}
...
$f(\text{total\_rate}(t), s)ds = \exp[-\text{total\_rate}(t)s]\times\text{total\_rate}(t)ds$. Therefore the time $\tau$, such that the next event happens at the time $t+\tau$, is exponentially distributed according to the probability distribution $f(\lambda, x)=\lambda\exp[-\lambda x]$ where $\lambda = \text{total\_rate}(t)$. \\
...
\bold{A}x=\lambda x
\end{equation}
where $\mathbf{A}$ denotes the networks’ adjacency matrix and $\lambda$ denotes the largest eigenvalue of $\mathbf{A}$, which is real and positive by the Perron-Frobenius theorem. Hence, $x$ is the eigenvector belonging to the spectral radius of $\mathbf{A}$. \cite{wiki:centrality}
\subsection{Implementation}
To simulate the infections, we used an implementation of the Gillespie algorithm from \cite{Code-Gillespie}, as well as Python functions, which enable us to plot the time behaviour of the infection. In addition we computed the footprint of the infection as a function of the effective strength. Our Code uses the libraries scipy, matplotlib, networkX, and EoN.
