...
We start by defining a vector $\bold{P}_t$ that will describe the system at time $t$.  In a model where $m$ is the total number of states and $n$ is the total number of nodes, this vector will have $m \times n$ entries, i.e. $\bold{P}_t \in \mathbb{R}^{m \cdot n} $. More specifically, we can write $\bold{P}_t$ in the following way: \\
\begin{eqnarray}
...
\textbf{TRANSLATE:} Dabei orinitieren wir uns an dem paper \cite{paper_gillespie} in welchem gezeigt wird wie der Gillespie-Algorithmus auf chemische Gleichungen angewandt werden kann.
\\
...
A Pseudocode for the Gillespie algorithm adopted to the SIR model is shown in Figure \ref{figure: Gillespie-pseudo}\\
\begin{figure}
\centering
